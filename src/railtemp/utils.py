import numpy as np
import pandas as pd
from math import sin, radians, cos, atan2, tan
...
def shadowArea_sunArea(input_df, sunAzimuth, sunElevation, railAzimuth):
    ...
    coord["xp"] = coord.apply(
        lambda k: project_point(k["X"], k["Y"], k["Z"], eqAzimtuth, sunElevation)[0], axis=1
    )
    coord["yp"] = coord.apply(
        lambda k: project_point(k["X"], k["Y"], k["Z"], eqAzimtuth, sunElevation)[1], axis=1
    )
